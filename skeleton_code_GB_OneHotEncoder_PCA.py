...
# Convert categorical variables ('urban_type', 'geography_type') using Label Encoding
...
train_cat = encoder.fit_transform(train_df[cat_features].astype(str))
test_cat = encoder.transform(test_df[cat_features].astype(str))  # No error for unseen categories

# Convert to DataFrame
print(encoder.get_feature_names_out(cat_features))
train_cat_df = pd.DataFrame(train_cat, columns=encoder.get_feature_names_out(cat_features))
test_cat_df = pd.DataFrame(test_cat, columns=encoder.get_feature_names_out(cat_features))

# Define the number of principal components (reduce dimensions)
pca = PCA(n_components=10)  # Adjust based on experiments

# Apply PCA on the one-hot encoded categorical data
train_cat_pca = pca.fit_transform(train_cat_df)
test_cat_pca = pca.transform(test_cat_df)

# Convert back to DataFrame
train_cat_df = pd.DataFrame(train_cat_pca, columns=[f'PCA_{i+1}' for i in range(train_cat_pca.shape[1])])
test_cat_df = pd.DataFrame(test_cat_pca, columns=[f'PCA_{i+1}' for i in range(test_cat_pca.shape[1])])

# Get updated column names after one-hot encoding
encoded_cat_features = list(train_cat_df.columns)  # Extracts new feature names

# Reset index
train_cat_df.index = train_df.index
test_cat_df.index = test_df.index

# Drop original categorical columns and replace with PCA-reduced features
train_df = train_df.drop(columns=cat_features).join(train_cat_df)
test_df = test_df.drop(columns=cat_features).join(test_cat_df)
