...
class GeoFeatureEngineer:
    ...
    def _calculate_geometric_features(self, df):
        ...
        # Razão de alongamento (elongação)
    ...
    def _calculate_spectral_features(self, df):
        ...
        for i in range(1, len(available_dates)):
                for band in ['red', 'green', 'blue']:
                    if f'img_{band}_mean_date{i}' in df.columns and f'img_{band}_mean_date{i-1}' in df.columns:
                        df[f'spectral_diff_{i}_{band}'] = df[f'img_{band}_mean_date{i}'] - df[f'img_{band}_mean_date{i-1}']

    # Criar NDVI (Índice de Vegetação Normalizado) para todas as datas disponíveis
# Criar um único NDVI baseado na primeira data disponível
        if available_dates:
            first_date = available_dates[0]
            if f'img_red_mean_date{first_date}' in df.columns and f'img_blue_mean_date{first_date}' in df.columns:
                df["NDVI"] = (df[f"img_red_mean_date{first_date}"] - df[f"img_blue_mean_date{first_date}"]) / \
                            (df[f"img_red_mean_date{first_date}"] + df[f"img_blue_mean_date{first_date}"] + 1e-9)
...
class CompetitionPipeline:
    ...
    # Tratar variáveis categóricas (usar One-Hot apenas para urban_geo_interaction)
        ...
        # Feature selection
        temporal_features = [c for c in self.train.columns if 'days_between' in c] + ['change_frequency', 'change_speed']
        spectral_features = [c for c in self.train.columns if 'spectral_' in c] + \
                            [c for c in self.train.columns if 'spectral_diff_' in c] + ['NDVI']
        geometric_features = ['area', 'perimeter', 'compactness', 'centroid_x', 'centroid_y', 
                            'bounding_width', 'bounding_height', 'elongation']
        categorical_features = ['urban_type_freq', 'geography_type_freq']  # Agora numéricas
    ...
    def train_ensemble(self):
        ...
        self.models = [
            ...
            #     random_state=RANDOM_STATE,
            #     early_stopping_rounds=50,
            #     task_type="CPU",  # ⚡ Força uso da CPU
            #     thread_count=-1  # Usa todos os núcleos na CPU
            # ))
